...
画像データ（数MB）と違い、「点群データ」は座標のテキスト（数値）だけなので非常に軽量です。
*   1つの破片 ≒ 2048個の点 ≒ **約 25KB**
    *   ※注: **3Dモデルのポリゴン数が増えても、ここから抽出する「点の数」は2048個に固定しているため、データ容量は一切増えません。**
*   1万個の破片データでも **約 250MB** 程度。Google Colabの無料枠でも余裕で扱えます。

### Q. なぜ「2048点」に減らすの？ 機械学習なら制限はないのでは？
**A. 「まとめて高速に計算する（バッチ処理）」ために、数を揃えるのが一般的です。**

確かに、最近のAI（Transformerなど）は可変長データを扱えるものもありますが、3D点群処理で最も使われる標準的なモデル（PointNet系）では、以下の理由で固定数が好まれます。

1.  **行列計算（Tensor）の都合**:
    *   AIの内部では、データを「固定サイズの表（行列）」として扱って計算します。
    *   例: `[バッチサイズ(32個), 点の数(2048個), 座標(3つ)]` という巨大なサイコロのような形で行列計算を行います。
    *   ここで1つだけ「10点」、隣は「5万点」とバラバラだと、きれいな行列が作れず、GPUで一気に計算できなくなります。
2.  **圧倒的な速度**:
    *   数を揃えることで、GPUのパワーをフルに使って「32個の破片」を同時に一瞬で判定できます。これを1個ずつ処理していると、学習に何ヶ月もかかってしまいます。

※もちろん、推論（本番）の時に、時間をかけてスライディングウィンドウ（少しずつずらして判定）する方法もありますが、学習時は「2048固定」にするのが世界標準の定石です。

### Q. 大きさ（何センチとか）は分かるの？ 縮尺が変わったら合わなくない？
**A. はい、実寸サイズのまま学習させます。**

おっしゃる通り、土器の復元において「大きさ（曲率）」は最も重要なヒントです。小さなカップの破片と、大きな甕（かめ）の破片では、カーブの緩やかさが全く違うからです。

1.  **データの作り方**:
    *   通常のAIではデータを「半径1の球」に縮小（正規化）してしまうことが多いですが、今回は**あえて正規化せず、Blender上の実寸座標（メートル単位）のまま**学習させます。
    *   これにより、AIは「このカーブは半径30cmの大きな壺の一部だ」「これは半径5cmの小さな突起だ」というサイズ感を含めて理解できます。
2.  **もし拡大縮小したら？**:
    *   これにより、AIは「このカーブは半径30cmの大きな壺の一部だ」「これは半径5cmの小さな突起だ」というサイズ感を含めて理解できます。
2.  **もし拡大縮小したら？**:
    *   写真と違い、3Dスキャンデータは「ミリメートル単位」で計測されるため、意図的に縮小しない限りサイズは維持されます。

### Q. 実寸にするとデータ容量は重くならない？
**A. 1バイトも重くなりません。**
コンピュータの中では、数字の大きさに関わらず「1つの座標＝32ビット（4バイト）」と決まっています。
*   `0.001` (正規化された値) → 32ビット
*   `30.5` (実寸の30.5cm) → **同じく32ビット**

ですので、実寸で記録してもファイルサイズは全く同じです。「他の人がやっているか」については、建築スキャン（ScanNet）やロボット工学など、サイズが重要な分野では当たり前に行われている方法です。
...
### Phase 2: AIモデルの選定と学習
1.  **モデル**: **PointNet++** または **DGCNN** (Dynamic Graph CNN)## 4. AIはどのように学習するのか？
AI（主にPointNet++やDGCNNなどの点群解析モデル）は、以下の3ステップで「パズル」を解く能力を身につけます。

### 1. 特徴抽出 (Feature Extraction)
*   **渡すデータ**: 各破片を数千点の点（点群 / Point Cloud）に変換したもの。
*   **学習内容**: AIは「断面の凹凸のパターン」を数値化（ベクトル化）します。
    *   「ここは急な崖になっている」「ここは緩やかな谷だ」という特徴を、点群の形状から読み取ります。

### 2. 対応関係の学習 (Contrastive Learning)
*   **渡すデータ**: 今回作成した「Facet IDのペア（正解）」
*   **学習内容**: 
    *   AIに2つの断面を見せ、「これはペア（正解）だよ」「これは違う（不正解）だよ」と教え込みます。
    *   **正解ペアの場合**: AIが計算した2つの特徴ベクトルが「近く」なるように調整します。
    *   **不正解ペアの場合**: 特徴ベクトルを「遠ざける」ように調整します。
*   **結果**: 学習が進むと、AIは未知の破片を見ても「この断面の形は、あっちの断面とベクトルが近いからペアだ！」と判断できるようになります。

### 3. 位置合わせの学習 (Pose Estimation)
*   **学習内容**: ペアだと分かった後、それらを「どういう角度で合体させるか（回転と移動）」を、最小二乗法やニューラルネットワークで予測します。
